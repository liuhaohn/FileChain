...
function printHelp() {
  ...
  echo "    -n <chaincode/contract name> - deployCC with this name, must same as the name in chaincode"
...
function deployCC() {
  ./scripts/deployCC.sh $CHANNEL_NAME $CC_SRC_LANGUAGE $VERSION $CLI_DELAY $MAX_RETRY $VERBOSE $CONTRACT_NAME
...
function networkDown() {
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    sudo rm -rf channel-artifacts log.txt filechain.tar.gz
