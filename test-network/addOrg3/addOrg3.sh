...
function addOrg3 () {
  ...
  # 这里认为本地docker中没有fabric-tools（客户端）容器就是没有启动网络，不正确
  # docker ps -a | awk '($2 ~ /fabric-tools/) {print $1}'
  # 改成没有org3的客户端
  CONTAINER_IDS=$(docker ps -a | awk '($2 ~ /fabric-tools/)' | awk '($0 ~ /org3/) {print $1}')
  ...
  echo "###############################################################"
  docker exec cli.org3.example.com ./scripts/org3-scripts/step1org3.sh $CHANNEL_NAME $CLI_DELAY $CLI_TIMEOUT $VERBOSE
  if [ $? -ne 0 ]; then
  ...
  echo "###############################################################"
  docker exec cli.org3.example.com ./scripts/org3-scripts/step2org3.sh $CHANNEL_NAME $CLI_DELAY $CLI_TIMEOUT $VERBOSE
  if [ $? -ne 0 ]; then
